...
class DbMagic(Magics):
    ...
    def parse_args(self,magic_args):
       ...
       have to infer a hell of a lot of things based on very little information.
        ...
        # print (self._args.source is not None) 
        # print (self._args.source in self.list_values( self._args.alias, self._args.type, 'sources')) 
        # print (self._args.cmd is not None) 
        # print (len(self._args.cmd) > 0) 
        # print (connection_key not in self._conn_info.keys())  
        # print (self._args.list is None) 
        # print (no_alias_provided)
        # print (self._args.connect) 
        # print (self._args.execute) 
        # print (self._args.disconnect)
        # print (imply_naked_query)
    ...
    def disconnect_from_source(self, connection_alias, connection_type):
        ...
        if connection_type.lower() == 'odbc':
            ...
            if not self.is_registered(connection_alias):
                ...
                logging.debug(" --- %s"  % self._conn_info)
                return
    ...
    def cleanup(self):
        ...
        for alias in self._conn_info.keys():
            try:
                ...
                logging.error(" - There was a problem shutting down %s" % alias)
